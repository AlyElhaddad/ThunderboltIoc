...
## 5.1. Services registered for you by default
...
### 5.2.1. Same service and implementation type
...
```
### 5.2.2. Different service and implementation types
For this scenario, you may register your services like:
```csharp
...
```
### 5.2.3. The service has a factory to determine the resulting instance
For this scenario, you may register your services like:
```csharp
...
### 5.2.4. The service has runtime logic to determine its implementation
For this scenario, you may register your services like:
```csharp
...
### 5.3.1. ThunderboltIncludeAttribute
...
### 5.3.2. ThunderboltExcludeAttribute
...
## 5.4. Regex-based attribute (convention) registration
...
### 5.4.1. ThunderboltRegexIncludeAttribute
...
#### 5.4.1.1. Registering services that they themselves represent their own implementation
Using only the two arguments discussed above is going to be sufficient for this scenario.
#### 5.4.1.2. Registering services that have different type for their implementation
...
##### 5.4.1.2.1 IFooService: FooService: For the common scenario where we may want to match IFooService with FooService, we may use the following:
```csharp
...
```
### 5.4.2. ThunderboltRegexExcludeAttribute
...
# 6. How to resolve/get your instances
...
## 6.1. Obtaining an IThunderboltContainer
...
## 6.2. Obtaining an IThunderboltScope
...
## 9.3. Verify no cyclic dependencies exist
...
## 9.7. Integration with Microsoft.Extensions.DependencyInjection
